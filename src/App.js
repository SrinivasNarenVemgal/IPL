...
const ALL_PLAYERS = Object.entries(SQUADS).flatMap(([t,ps])=>ps.map(n=>({name:n,team:t}))).sort((a,b)=>a.name.localeCompare(b.name));

const IPL_TEAMS = [
  {name:"Mumbai Indians",              short:"MI",  color:"#004BA0",accent:"#D1AB3E"},
  {name:"Chennai Super Kings",         short:"CSK", color:"#C8960C",accent:"#0081E9"},
  {name:"Royal Challengers Bengaluru", short:"RCB", color:"#CC0000",accent:"#333"},
...
export default function App(){
  ...
  const botMsg = (text, delay=650, onDone) => {
    ...
    setTimeout(()=>{
      setTyping(false);
      setMsgs(prev=>[...prev,{from:"bot",text,time:nowTime(),id:Math.random()}]);
      if(onDone) onDone();
    }, delay);
  };
    ...
    if(pinInput===pendingWho.pin){
      ...
      if(pendingWho.name===ADMIN_NAME){
        ...
        if(unpredicted.length===1){
          setCurMatch(unpredicted[0]);setStep(0);setMsgs([]);setTyping(false);
    ...
    return(
      <div style={{position:"relative"}}>
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          <input style={adminInp} placeholder="Type player name..." value={q}
            onChange={e=>{setQ(e.target.value);setOpen(true);onChange("");}}
            onFocus={()=>setOpen(true)} onBlur={()=>setTimeout(()=>setOpen(false),180)}/>
          {q&&<button onClick={()=>{onChange("");setQ("");}} style={{background:"transparent",border:"none",color:OR,fontSize:18,cursor:"pointer"}}>×</button>}
        </div>
        {value&&!open&&<div style={{marginTop:5,display:"flex",alignItems:"center",gap:8,background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.3)",borderRadius:10,padding:"5px 12px"}}>
          <div style={{width:18,height:18,borderRadius:"50%",background:tC(ALL_PLAYERS.find(p=>p.name===value)?.team||""),flexShrink:0}}/>
        ...
        {open&&fl.length>0&&<div style={{position:"absolute",top:"100%",left:0,right:0,background:"#1a1a2e",border:"1px solid rgba(255,165,0,0.25)",borderRadius:12,boxShadow:"0 8px 24px rgba(0,0,0,0.6)",zIndex:999,maxHeight:200,overflowY:"auto",marginTop:4}}>
          {fl.map((p,i)=><div key={i} onMouseDown={()=>{onChange(p.name);setQ(p.name);setOpen(false);}}
            ...
            <div style={{width:22,height:22,borderRadius:"50%",background:tC(p.team),display:"flex",alignItems:"center",justifyContent:"center",fontSize:7,fontWeight:800,color:"#fff"}}>{p.team}</div>
      ...
      {screen===SCREENS.LAND&&(
        <div style={{minHeight:"100vh",background:"#075E54",fontFamily:"Poppins,sans-serif",display:"flex",flexDirection:"column"}}>
          ...
          <div style={{flex:1,background:"#fff",overflowY:"auto"}}>
            <div style={{padding:"9px 16px 6px",fontSize:11,fontWeight:700,color:"#075E54",letterSpacing:1,textTransform:"uppercase",background:"#f0f0f0",borderBottom:"1px solid #e0e0e0"}}>
            ...
            {MEMBERS.map((m,i)=>{
              ...
              return(
                <button key={i} onClick={()=>{setPendingWho(m);setPinInput("");setPinErr("");setScreen(SCREENS.PIN);}}
                  ...
                  <div style={{width:48,height:48,borderRadius:"50%",background:"linear-gradient(135deg,#25D366,#128C7E)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{emo(m.name)}</div>
        ...
        <div style={{minHeight:"100vh",background:"#0D1B3E",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 16px",fontFamily:"Poppins,sans-serif"}}>
          <div style={{width:"100%",maxWidth:310,animation:"fadeUp 0.3s ease"}}>
            ...
            <div style={{background:"rgba(255,255,255,0.07)",borderRadius:20,padding:"20px 18px",border:"1px solid rgba(255,255,255,0.12)",animation:pinShake?"shake 0.4s ease":"none"}}>
              ...
              <input style={{...navInp,textAlign:"center",letterSpacing:10,fontSize:20,marginBottom:8}}
          ...
          <div style={{background:WA_HEADER,padding:"14px 16px",position:"sticky",top:0,zIndex:10,boxShadow:"0 1px 6px rgba(0,0,0,0.3)"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,color:"rgba(255,255,255,0.65)"}}>IPL 2026 Predictions</div>
              </div>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      <div style={{fontWeight:700,fontSize:15,color:"#111"}}>
                        {t1} vs {t2}
                        {isDouble&&<span style={{fontSize:10,fontWeight:700,color:WA_HEADER,background:"#e8f5e9",padding:"1px 6px",borderRadius:8,marginLeft:6}}>Match {idx+1}</span>}
                      </div>
              ...
              <div>
                {filtered.length>0&&(
                  <div style={{background:"#fff",borderTop:"1px solid #eee",maxHeight:190,overflowY:"auto"}}>
                    {filtered.map((p,i)=>{
                      const tColor=IPL_TEAMS.find(x=>x.short===p.team)?.color||"#555";
                      return(
                        <div key={i} onMouseDown={()=>{answer(p.name,p.name);setSearch("");}}
                          ...
                          onMouseLeave={e=>e.currentTarget.style.background="#fff"}>
                          <div style={{width:26,height:26,borderRadius:"50%",background:tColor,display:"flex",alignItems:"center",justifyContent:"center",fontSize:7,fontWeight:800,color:"#fff",flexShrink:0}}>{p.team}</div>
                          <span style={{fontSize:14,fontWeight:600,color:"#222",flex:1}}>{p.name}</span>
                          <span style={{fontSize:11,color:"#aaa"}}>{p.team}</span>
                        </div>
                      );
                    })}
                  </div>
          ...
          <div style={{maxWidth:520,margin:"0 auto",padding:"18px 16px"}}>
            <div style={aCard}>
              ...
              <div style={{marginBottom:14}}><label style={aLbl}>🔒 Lock At <span style={{color:"#22c55e",fontSize:10,fontWeight:400,textTransform:"none"}}>(auto-set)</span></label>
                ...
                <div style={{fontSize:11,color:"#444",marginTop:4}}>Evening = 7:30 PM · Sat/Sun afternoon = 3:30 PM</div>
              </div>
                ...
                return(
                  <div key={m.id} style={{background:"rgba(255,255,255,0.03)",borderRadius:12,padding:"10px 12px",marginBottom:8,border:"1px solid rgba(255,255,255,0.05)"}}>
                    ...
                    <div style={{display:"flex",gap:6,marginTop:8}}>
                      <button onClick={()=>{setAdminMatch(m);setRForm({tossWinner:"",tossDecision:"",winner:"",topScorer:"",topWicket:"",manOfMatch:"",sixHitter:"",pp1:"",pp2:""});setScreen(SCREENS.RESULTS);}}
                      ...
                      {done&&<button onClick={()=>{setAdminMatch(m);setScreen(SCREENS.COMPARE);}}
                        style={{flex:1,padding:"7px",borderRadius:8,border:"none",background:"rgba(34,197,94,0.12)",color:"#22c55e",fontSize:11,fontFamily:"Poppins,sans-serif",fontWeight:600,cursor:"pointer"}}>
                        📊 Compare
                      </button>}
                    </div>
                  </div>
      ...
      {screen===SCREENS.RESULTS&&who?.name===ADMIN_NAME&&adminMatch&&(
        <div style={{minHeight:"100vh",background:"#080810",color:"#fff",fontFamily:"Poppins,sans-serif",paddingBottom:40}}>
          <div style={{background:"rgba(255,255,255,0.04)",borderBottom:"1px solid rgba(255,165,0,0.2)",padding:"14px 20px",display:"flex",gap:12,alignItems:"center"}}>
            <button onClick={()=>setScreen(SCREENS.ADMIN)} style={{padding:"7px 14px",borderRadius:8,border:"1px solid rgba(255,165,0,0.3)",background:"transparent",color:OR,fontSize:12,cursor:"pointer"}}>← Back</button>
          ...
          <div style={{maxWidth:520,margin:"0 auto",padding:"16px"}}>
            <div style={{...aCard,textAlign:"center",marginBottom:12}}>
              <div style={{display:"flex",justifyContent:"center",alignItems:"center",gap:16}}>
                <Ball name={adminMatch.team1} size={46}/><div style={{color:OR,fontFamily:"'Bebas Neue',sans-serif",fontSize:20}}>VS</div><Ball name={adminMatch.team2} size={46}/>
              </div>
